...
export class PromotionsService {
    ...
    async createPromotion(createPromotionDto: CreatePromotionDto, shop: Shop) {
        if (createPromotionDto.startDate && createPromotionDto.endDate && createPromotionDto.startDate > createPromotionDto.endDate) 
        ...
        for (let i = 0; i < createPromotionDto.products.length; i++) {
            ...
            const product = await this.productModel.findOne({ _id: createPromotionDto.products[i], shop: shop._id });
    ...
    async updatePromotion(id: string, updatePromotionDto: UpdatePromotionDto, shop: Shop) {
        validateObjectId(id, 'Promotion');
        const promotion = await this.promotionModel.findOne({ _id: id, shop: shop._id });
        ...
        if (updatePromotionDto.hasOwnProperty("products")) {
            for (let i = 0; i < updatePromotionDto.products.length; i++) {
                ...
                const product = await this.productModel.findOne({ _id: updatePromotionDto.products[i], shop: shop._id });
                if (!product) throw new HttpException('Product not found', 404);
            }
        }

        if (updatePromotionDto.hasOwnProperty("startDate") && updatePromotionDto.hasOwnProperty("endDate") && updatePromotionDto.startDate > updatePromotionDto.endDate) {
            throw new HttpException('Start date must be before end date', 400);
        } else if ( updatePromotionDto.hasOwnProperty("startDate") && updatePromotionDto.hasOwnProperty("endDate") && promotion.hasOwnProperty("endDate")) {
            if (updatePromotionDto.endDate < promotion.startDate) throw new HttpException('End date must be after start date', 400);
        } else if (updatePromotionDto.hasOwnProperty("startDate") &&  updatePromotionDto.hasOwnProperty("endDate") && promotion.hasOwnProperty("startDate")) {
            if (updatePromotionDto.startDate > promotion.endDate) throw new HttpException('Start date must be before end date', 400); 
        }
        const data = { ...updatePromotionDto };

        return this.promotionModel.findOneAndUpdate({ _id: id, shop: shop._id }, data, { new: true });
    }
