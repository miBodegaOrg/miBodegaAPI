...
export class EmployeesService {
    ...
    async createEmployee(createEmployeeDto: CreateEmployeeDto, shop: Shop) {
        const employee = await this.employeeModel.findOne({ email: createEmployeeDto.email, shop: shop._id });
        if (employee) throw new HttpException('Employee with this email already exists', 400);
