...
export class SuppliersService {
    ...
    async create(createSupplierDto: CreateSupplierDto, shop: Shop) {
        ...
        for (let i = 0; i < createSupplierDto.products.length; i++) {
            ...
            if (product.supplier) throw new HttpException(`Product with code ${createSupplierDto.products[i].code} already has a supplier`, 400);
      ...
      const supplier = await this.supplierModel.findOne({ ruc, shop: shop._id });
        ...
        if (updateSupplierDto.products) {
            for (let i = 0; i < updateSupplierDto.products.length; i++) {
                let product = await this.productModel.findOne({ code: updateSupplierDto.products[i].code, shop: shop._id })
                if (!product) throw new HttpException(`Product with code ${updateSupplierDto.products[i].code} not found`, 404);
                if (product.supplier && product.supplier != supplier._id) throw new HttpException(`Product with code ${updateSupplierDto.products[i].code} already has a supplier`, 400);

                updateSupplierDto.products[i] = { ...updateSupplierDto.products[i], _id: product._id };
            }
        }
